        ...
        getFile: {
            ...
            async value(PATH, saveToFile = false) {
                ...
                if (data && saveToFile && !filename)
                    (filename = path.join(__dirname, '../tmp/' + new Date().getTime() + '.' + type.ext), await fs.promises.writeFile(filename, data));
                return {
                    res,
                    filename,
                    ...type,
...
END:VCARD`.trim();
        ...
        sendStvid: {
            async value(jid, path, quoted, options = {}) {
                let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split `,`[1], 'base64') : /^https?:\/\//.test(path) ? await global.getBuffer(path) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0);
                let buffer;
                if (options && (options.packname || options.author)) {
...
export function protoType() {
    ...
    }(String.prototype).isNumber = Number.prototype.isNumber = isNumber(String.prototype).capitalize = function capitalize() {
