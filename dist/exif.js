...
async function imageToWebp(media) {
    const tmpFileOut = (0, path_1.join)((0, os_1.tmpdir)(), `${Math.random().toString(36).substring(2, 12)}.webp`);
    const tmpFileIn = (0, path_1.join)((0, os_1.tmpdir)(), `${Math.random().toString(36).substring(2, 12)}.tmp`);
    (0, fs_1.writeFileSync)(tmpFileIn, media);
    await new Promise((resolve, reject) => {
        (0, child_process_1.exec)(`ffmpeg -i ${tmpFileIn} -vcodec libwebp -vf \"scale=\'min(320,iw)\':\'min(320,ih)\':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split[a][b]; [a]palettegen[p]; [b][p]paletteuse\" -loop 0 -ss 00:00:00.0 -t 00:00:05.0 -preset default -an -vsync 0 -s 512:512 ${tmpFileOut}`, (error, stdout, stderr) => {
            if (error)
                return reject(error);
            resolve(true);
        });
    });
    const buff = (0, fs_1.readFileSync)(tmpFileOut);
    (0, fs_1.unlinkSync)(tmpFileOut);
    (0, fs_1.unlinkSync)(tmpFileIn);
    return buff;
}
