...
exports.capitalizeV2 = capitalizeV2;
...
async function generateProfilePicture(buffer) {
    ...
    return {
        ...
        preview: await cropped.scaleToFit(72, 72).getBufferAsync(jimp_1.default.MIME_JPEG)
    };
}
...
function smsg(conn, m, chatUpdate = {}) {
    if (!m)
        return m;
    let M = baileys_1.proto.WebMessageInfo;
    if (m.key) {
        m.id = m.key.id;
...
function writeExif(media, metadata) {
    let wMedia = /image/.test(media.mimetype) ? imageToWebp : videoToWebp;
    return new Promise(async (resolve, reject) => {
        const FAKE_MEDIA_PATH = path_1.default.join(tmpdir(), getRandom('.webp'));
        const FAKE_FILE_PATH = path_1.default.join(tmpdir(), getRandom('.json'));
        const FAKE_IMG_PATH = path_1.default.join(tmpdir(), getRandom('.png'));
        const FAKE_STICKER_PATH = path_1.default.join(tmpdir(), getRandom('.webp'));
        const FAKE_RESULT_PATH = path_1.default.join(tmpdir(), getRandom('.webp'));
        ...
        await fs_1.default.promises.writeFile(FAKE_MEDIA_PATH, await wMedia(media.data));
        await fs_1.default.promises.writeFile(FAKE_FILE_PATH, fullBuff);
        const error = await spawn('webpmux', [
        ...
        if (error)
            reject(error);
        fs_1.default.promises.unlink(FAKE_MEDIA_PATH);
        fs_1.default.promises.unlink(FAKE_FILE_PATH);
        resolve(FAKE_RESULT_PATH);
    });
}
function imageToWebp(buffer) {
    return new Promise((resolve, reject) => {
        spawn('convert', ['-quality', '100', '-', 'webp:-'], { stdio: ['pipe', 'pipe', 'ignore'] })
            .on('error', reject)
            .on('close', (code) => {
            if (code !== 0)
